 ...
 * @param {string} dohURL The DNS over HTTPS URL.
 * @returns {{write: (chunk: Uint8Array) => void}} An object with a write method that accepts a Uint8Array chunk to write to the transform stream.
 */
async function handleUDPOutBound(webSocket, vlessResponseHeader, log, dohURL) {
 ...
 * @param {Uint8Array | boolean} vlessResponseHeader The VLESS response header.
...
async function parseTrojanHeader(buffer, hashPassword) {
 ...
 * @param {string} dohURL The DNS over HTTPS URL.
 * @param {string} proxyIP The proxy IP.
 * @returns {Promise<Response>} A Promise that resolves to a WebSocket response object.
 */
export async function vlessOverWSHandler(request, userID, dohURL, proxyIP) {
    ...
    .pipeTo(
        new WritableStream({
            async write(chunk, controller) {
                ...
                if (isDns) {
                    const { write } = await handleUDPOutBound(webSocket, vlessResponseHeader, log, dohURL);
...
export async function trojanOverWSHandler(request, hashPassword, proxyIP) {
    const webSocketPair = new WebSocketPair();
    const [client, webSocket] = Object.values(webSocketPair);
    webSocket.accept();
    let address = "";
            ...
            new WritableStream({
                async write(chunk, controller) {
                    ...
                    const {
                        ...
                        addressRemote = "",
                        rawClientData,
                    } = await parseTrojanHeader(chunk, hashPassword);
                    ...
                    handleTCPOutBound(request, remoteSocketWapper, addressRemote, portRemote, rawClientData, webSocket, false, log, proxyIP);
                },
                close() {
